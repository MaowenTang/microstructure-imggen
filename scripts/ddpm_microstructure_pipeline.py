...
"""
ddpm_microstructure_pipeline.py
...
- OOM avoidance: attention only at low-res (16x16; optional 32x32)
"""
...
def build_unet(resolution: int, attn_level: str = "16"):
    """
    attn_level options (for 512 training):
      - "none": no attention (fastest, least sharp)
      - "16":   attention only at lowest-res (safest)            [your current default]
      - "32":   attention at 32x32 and 16x16                     (stronger, moderate cost)
      - "64":   attention at 64x64, 32x32, 16x16                 (sharper, higher VRAM)
      - "128":  attention at 128x128, 64x64, 32x32, 16x16        (very heavy; may OOM)
      - "256":  attention at 256x256 ... (almost surely OOM at 512; kept for completeness)
      - "all":  attention at every block (max; usually OOM)
    Notes:
      UNet2DModel with 5 blocks typically corresponds to feature resolutions roughly:
        b0: 256, b1: 128, b2: 64, b3: 32, b4: 16   (starting from 512 input)
      So “put attention at 64” means: make b2/b3/b4 attentional.
    """
    ...
    if attn_level == "none":
        ...
        down = ("DownBlock2D", "AttnDownBlock2D", "AttnDownBlock2D", "AttnDownBlock2D", "AttnDownBlock2D")
        up   = ("AttnUpBlock2D", "AttnUpBlock2D", "AttnUpBlock2D", "AttnUpBlock2D", "UpBlock2D")

    elif attn_level == "256":
        # 256x256 + 128x128 + 64x64 + 32x32 + 16x16  (very likely OOM)
        down = ("AttnDownBlock2D", "AttnDownBlock2D", "AttnDownBlock2D", "AttnDownBlock2D", "AttnDownBlock2D")
        up   = ("AttnUpBlock2D", "AttnUpBlock2D", "AttnUpBlock2D", "AttnUpBlock2D", "AttnUpBlock2D")

    elif attn_level == "all":
        # same as 256 here (all blocks)
        down = ("AttnDownBlock2D",) * 5
        up   = ("AttnUpBlock2D",) * 5
...
def run_train_ddpm(args):
    ...
    while step < args.max_steps:
        if sampler is not None:
            # keeps shuffle different across ranks; cheap call
        ...
        for micro in range(args.grad_accum):
            ...
            # use scheduler.config.num_train_timesteps to avoid deprecation warning
        ...
        if is_main and (step % args.log_every == 0):
            ...
            # total_loss is already divided by grad_accum; multiply back for a more interpretable "per-batch" loss
...
def build_parser():
    ...
    p_tr.add_argument(
        ...
        type=str,
        default="16",
        choices=["none", "16", "32", "64", "128", "256", "all"],
        help="Where to use self-attention. '16' is safest for 512. '32' stronger but heavier.",
    )
