...
"""
ldm_microstructure_pipeline_v2.py
...
Commands:
  ...
  2) train_vae      : Train a lightweight VAE -> latent (z_channels, 64, 64)
  3) recon_vae      : Visualize VAE reconstructions (GT vs recon)
  4) train_ldm      : Train DDPM UNet in latent space (64x64)
  5) sample         : Sample from latent DDPM and decode by VAE

DDP:
  torchrun --nproc_per_node=4 ldm_microstructure_pipeline_v2.py train_vae ...
  torchrun --nproc_per_node=4 ldm_microstructure_pipeline_v2.py train_ldm ...
"""
...
class PatchFolder(Dataset):
    def __init__(self, root: str, resolution: int = 512, flip: bool = True):
        self.paths = list_images(os.path.expanduser(root))
        if len(self.paths) == 0:
...
def vae_recon_grid(vae, dl, device, out_png, n=9):
    ...
    # one sample = left GT, right recon
    canvas = Image.new("RGB", (cols * 1024, rows * 512))
    for i in range(n):
        r, c = divmod(i, cols)
        im0 = tensor_to_pil_m11(x[i])
        im1 = tensor_to_pil_m11(x_rec[i])
        canvas.paste(im0, (c * 1024, r * 512))
        canvas.paste(im1, (c * 1024 + 512, r * 512))
...
def cmd_recon_vae(args):
    ...
    ds = PatchFolder(data_root, resolution=512, flip=False)
    dl = DataLoader(ds, batch_size=args.n, shuffle=True, num_workers=0)
...
@torch.no_grad()
def sample_ldm(step, vae, unet, scheduler, device, out_png, z_channels=4, n=9, infer_steps=250):
    vae.eval()
    unet.eval()

    z = torch.randn(n, z_channels, 64, 64, device=device)
...
def build_latent_unet(z_channels=4):
    return UNet2DModel(
        sample_size=64,
...
def cmd_train_ldm(args):
    ...
    unet = build_latent_unet(z_channels=args.z_channels).to(device)
    ...
    log0(f"[LDM] z_channels={args.z_channels}  amp={args.amp} grad_accum={accum}  steps={args.max_steps}")
    ...
    while step < args.max_steps:
        ...
        for x0 in dl:
            ...
            if is_main() and step > 0 and step % args.sample_every == 0:
                ...
                sample_ldm(step, vae, base_unet, scheduler, device, out_png,
                           z_channels=args.z_channels, n=9, infer_steps=args.infer_steps)
...
def cmd_sample(args):
    ...
    vae_ckpt = os.path.expanduser(args.vae_ckpt)
    ldm_ckpt = os.path.expanduser(args.ldm_ckpt)

    if not os.path.exists(ldm_ckpt):
        raise FileNotFoundError(
            f"LDM ckpt not found: {ldm_ckpt}\n"
            f"Hint: run train_ldm first, or check --out_root and saved filename."
        )
    ...
    sample_ldm(
        0, vae, unet, scheduler, device, out_png,
        z_channels=args.z_channels, n=args.n, infer_steps=args.infer_steps
    )
